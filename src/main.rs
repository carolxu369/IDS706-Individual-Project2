...
fn main() -> Result<(), Box<dyn Error>> {
    let matches = App::new("My CLI App")
        ...
        .subcommand(
            SubCommand::with_name("create")
                ...
                .arg(Arg::with_name("DATA").help("The data to be inserted").required(true)),
        )
        .subcommand(
        ...
        .subcommand(
            SubCommand::with_name("update")
                ...
                .arg(Arg::with_name("ID").help("The ID of the record to update").required(true))
                .arg(Arg::with_name("DATA").help("The new data for the record").required(true)),
        )
        .subcommand(
            SubCommand::with_name("delete")
                .about("Deletes a record")
                .arg(Arg::with_name("ID").help("The ID of the record to delete").required(true)),
        )
        .get_matches();
    ...
    match matches.subcommand() {
        ("create", Some(sub_m)) => {
            let data = sub_m.value_of("DATA").ok_or(CliError::ArgumentError("Missing data for create".into()))?;
